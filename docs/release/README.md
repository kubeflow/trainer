...
1. Update the changelog by running:
   ...
   Check this example: [v2.0.0-rc.0](https://github.com/kubeflow/trainer/blob/master/CHANGELOG/changelog-2.0.md)
...
3. Update:
   ...
   - `CHANGELOG/changelog-X.Y.md` (prepends unreleased section using `git-cliff`)
4. Run `make generate`.
...
Review `VERSION`, `api/python_api/kubeflow_trainer_api/__init__.py`, and `CHANGELOG/changelog-X.Y.md`
to ensure correctness.

- **For latest minor series**: Open a PR to `main` and get it reviewed and merged.
- **For older minor series patch** (e.g. `2.0.1` when `main` is at `2.1.x`):
  Checkout the `release-X.Y` branch, commit changes and then open a PR to the
  corresponding `release-X.Y` branch.
