...
type XGBoostMLPolicySource struct {
    ...
    // For CPU training, this is typically 1 (default).
    // For GPU training, set this to the number of GPUs per node.
...
spec:
  mlPolicy:
    numNodes: 4
    xgboost:
      numWorkersPerNode: 1  # Default for CPU
```

#### GPU Training

> [!IMPORTANT]
> **XGBoost 2.0+ uses one worker per GPU.** Unlike the deprecated `n_gpus` parameter (removed in XGBoost 2.0), modern XGBoost requires launching one worker process per GPU for distributed GPU training.

- Set `device="cuda"` in XGBoost params
- **One worker per GPU** — same pattern as PyTorch, Ray, Dask-CUDA, and Spark
- `numWorkersPerNode` should equal the number of GPUs per node
...
spec:
  mlPolicy:
    numNodes: 2
    xgboost:
      numWorkersPerNode: 4  # One worker per GPU
  template:
    spec:
      replicatedJobs:
        - name: node
...
| Configuration | numNodes | numWorkersPerNode | DMLC_NUM_WORKER | Total Pods |
|:--------------|:---------|:------------------|:----------------|:-----------|
| 4 nodes, CPU-only | 4 | 1 (default) | 4 | 4 |
| 2 nodes, 4 GPUs each | 2 | 4 | 8 | 8 |
| 1 node, 8 GPUs | 1 | 8 | 8 | 8 |

```python
# CPU: uses all available cores, or limit with nthread
params = {"nthread": 8, "tree_method": "hist"}

# GPU: each worker uses exactly one GPU
params = {"device": "cuda", "tree_method": "hist"}
```

> **Note:** The `numWorkersPerNode` field controls how many worker pods are created per logical node. For GPU training, this should match the number of GPUs available on each physical node. NCCL is used for GPU-to-GPU communication when multiple GPUs are present.
...
flowchart TB
    subgraph Kubernetes Cluster
        subgraph Headless Service
            ...
            W1["Pod: myjob-node-1-0<br/>DMLC_TASK_ID=1"]
            W2["Pod: myjob-node-2-0<br/>DMLC_TASK_ID=2"]
            W3["Pod: myjob-node-3-0<br/>DMLC_TASK_ID=3"]
        end
    end
...
- Pattern: `<trainjob-name>-node-<replica-index>-<pod-index>`
- Tracker runs on the first pod: `<trainjob-name>-node-0-0`

**Multi-GPU Example (2 nodes × 2 GPUs each):**
```
myjob-node-0-0  DMLC_TASK_ID=0  (Tracker)
myjob-node-0-1  DMLC_TASK_ID=1
myjob-node-1-0  DMLC_TASK_ID=2
myjob-node-1-1  DMLC_TASK_ID=3
```
...
spec:
  ...
  # For multi-GPU per node, set numWorkersPerNode to match GPUs per node
  mlPolicy:
    xgboost:
      numWorkersPerNode: 4         # Creates 4 worker pods per node
# Total workers = numNodes × numWorkersPerNode
```
...
│   ├── Override numNodes and numWorkersPerNode from TrainJob if specified
│   ├── Calculate total workers: numNodes × numWorkersPerNode
│   ├── Find trainer container by ancestor label
│   └── Inject DMLC_* environment variables:
│       ├── DMLC_NUM_WORKER = numNodes × numWorkersPerNode
│       ├── DMLC_TASK_ID = Global worker rank (pod index)
...
- Fixed deprecated APIs and parallelism model: January 28th 2026
