...
func GenerateTrialName(optJobName string, params []trainer.ParameterAssignment) string {
	return generateTrialName(optJobName, nil, params)
}

// GenerateTrialNameWithIndex generates a deterministic Kubernetes name for a
// trial. The index distinguishes trials that receive the same parameter
// assignments, which is possible for finite categorical search spaces.
func GenerateTrialNameWithIndex(optJobName string, trialIndex int32, params []trainer.ParameterAssignment) string {
	return generateTrialName(optJobName, &trialIndex, params)
}

func generateTrialName(optJobName string, trialIndex *int32, params []trainer.ParameterAssignment) string {
	...
	suffix := "-trial-"
	if trialIndex != nil {
		hashInput = fmt.Sprintf("%s-%d", hashInput, *trialIndex)
		suffix = fmt.Sprintf("-trial-%d-", *trialIndex)
	}
	hash := sha256.Sum256([]byte(hashInput))
	shortHash := hex.EncodeToString(hash[:])[:8]

	maxPrefix := 63 - len(suffix) - len(shortHash)
	prefix := optJobName
	if len(prefix) > maxPrefix {
		prefix = strings.TrimRight(prefix[:maxPrefix], "-")
	}

	return fmt.Sprintf("%s%s%s", prefix, suffix, shortHash)
}
