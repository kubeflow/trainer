...
func TestGenerateTrialName(t *testing.T) {
	...
	t.Run("indexed names distinguish duplicate parameter assignments", func(t *testing.T) {
		params := []trainer.ParameterAssignment{{Name: "batch_size", Value: "32"}}
		name1 := GenerateTrialNameWithIndex("my-optjob", 0, params)
		name2 := GenerateTrialNameWithIndex("my-optjob", 1, params)
		if name1 == name2 {
			t.Fatalf("indexed trial names must differ for duplicate assignments: %q", name1)
		}
		if len(name1) > 63 || len(name2) > 63 {
			t.Fatalf("indexed trial name exceeds 63 characters: %q, %q", name1, name2)
		}
	})
