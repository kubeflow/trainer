	...
	cases := map[string]struct {
		...
		"plainml MLPolicy is applied to runtime.Info, TrainJob doesn't have numNodes": {
			...
			runtimeInfo: runtime.NewInfo(
				...
				runtime.WithPodSpecReplicas(constants.DatasetInitializer, 1, nil, corev1ac.PodSpec().
					WithContainers(
						corev1ac.Container().WithName(constants.DatasetInitializer),
					),
				),
				runtime.WithPodSpecReplicas(constants.ModelInitializer, 1, nil, corev1ac.PodSpec().
					WithContainers(
			...
			wantRuntimeInfo: &runtime.Info{
				...
				Scheduler: &runtime.Scheduler{
					TotalRequests: map[string]runtime.TotalResourceRequest{
						constants.DatasetInitializer: {Replicas: 1},
						constants.ModelInitializer:   {Replicas: 1},
	...
	cases := map[string]struct {
		...
		"succeeded to build PodGroup and JobSet with NumNodes from TrainJob": {
			...
			runtimeInfo: &runtime.Info{
				...
				Scheduler: &runtime.Scheduler{
					TotalRequests: map[string]runtime.TotalResourceRequest{
						constants.DatasetInitializer: {
							Replicas:    1,
							PodRequests: resRequests,
						},
						constants.ModelInitializer: {
							Replicas:    1,
							PodRequests: resRequests,
						},
