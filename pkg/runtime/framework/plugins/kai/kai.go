...
// KAI represents the NVIDIA KAI Scheduler plugin for gang-scheduling.
...
var _ framework.EnforcePodGroupPolicyPlugin = (*KAI)(nil)
var _ framework.WatchExtensionPlugin = (*KAI)(nil)
var _ framework.ComponentBuilderPlugin = (*KAI)(nil)

const Name = "KAI"

// PodGroupLabel is the label used to associate pods with a PodGroup for gang-scheduling.
const PodGroupLabel = "scheduling.kai.io/pod-group"

// +kubebuilder:rbac:groups=scheduling.kai.io,resources=queues,verbs=create;get;list;watch;update;patch
...
func (k *KAI) Name() string {
	return Name
}

func (k *KAI) EnforcePodGroupPolicy(info *runtime.Info, trainJob *trainer.TrainJob) error {
	if info == nil || info.RuntimePolicy.PodGroupPolicy == nil || info.RuntimePolicy.PodGroupPolicy.KAI == nil || trainJob == nil {
		return nil
	}
	...
	// Use KAI Scheduler's pod group label for gang-scheduling
	info.Scheduler.PodLabels[PodGroupLabel] = trainJob.Name

	// Set queue name if specified
	if info.RuntimePolicy.PodGroupPolicy.KAI.QueueName != nil {
		info.Scheduler.PodLabels["scheduling.kai.io/queue"] = *info.RuntimePolicy.PodGroupPolicy.KAI.QueueName
	}

	return nil
}

func (k *KAI) Build(ctx context.Context, info *runtime.Info, trainJob *trainer.TrainJob) ([]apiruntime.ApplyConfiguration, error) {
	if info == nil || info.RuntimePolicy.PodGroupPolicy == nil || info.RuntimePolicy.PodGroupPolicy.KAI == nil || trainJob == nil {
		return nil, nil
	}

	// Do not update the PodGroup if it already exists and the TrainJob is not suspended
	// Using scheduler-plugins PodGroup as KAI Scheduler is compatible with it
	...
	// Build PodGroup spec for KAI Scheduler
	podGroup := schedulerpluginsv1alpha1ac.PodGroup(trainJob.Name, trainJob.Namespace)

	scheduleTimeout := int32(60)
	if info.RuntimePolicy.PodGroupPolicy.KAI.ScheduleTimeoutSeconds != nil {
		scheduleTimeout = *info.RuntimePolicy.PodGroupPolicy.KAI.ScheduleTimeoutSeconds
	}

	podGroup.WithSpec(schedulerpluginsv1alpha1ac.PodGroupSpec().
		WithMinMember(totalMembers).
		WithMinResources(totalResources).
		WithScheduleTimeoutSeconds(scheduleTimeout))
...
func (k *KAI) ReconcilerBuilders() []runtime.ReconcilerBuilder {
	if _, err := k.restMapper.RESTMapping(
		...
		k.logger.Error(err, "PodGroup CRDs must be installed in advance")
		return nil
	}
		...
		func(b *builder.Builder, cl client.Client, cache cache.Cache) *builder.Builder {
			return b.Watches(
				...
				handler.EnqueueRequestForOwner(
					k.client.Scheme(), k.client.RESTMapper(), &trainer.TrainJob{}, handler.OnlyControllerOwner(),
				),
			)
		},
		func(b *builder.Builder, cl client.Client, cache cache.Cache) *builder.Builder {
