...
func TestKAI(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"succeeded to build PodGroup with KAI scheduler": {
			info: &runtime.Info{
				Scheduler: &runtime.Scheduler{},
				RuntimePolicy: runtime.RuntimePolicy{
					PodGroupPolicy: &trainerv1alpha1.PodGroupPolicy{
						PodGroupPolicySource: trainerv1alpha1.PodGroupPolicySource{
							KAI: &trainerv1alpha1.KAIPodGroupPolicySource{
		...
		"succeeded to build PodGroup with KAI queue name": {
			info: &runtime.Info{
				Scheduler: &runtime.Scheduler{},
				RuntimePolicy: runtime.RuntimePolicy{
					PodGroupPolicy: &trainerv1alpha1.PodGroupPolicy{
						PodGroupPolicySource: trainerv1alpha1.PodGroupPolicySource{
							KAI: &trainerv1alpha1.KAIPodGroupPolicySource{
								QueueName:              ptr.To("training-queue"),
			...
			wantInfo: &runtime.Info{
				Scheduler: &runtime.Scheduler{
					PodLabels: map[string]string{
						PodGroupLabel:             "trainJob",
						"scheduling.kai.io/queue": "training-queue",
					},
				},
				RuntimePolicy: runtime.RuntimePolicy{
					PodGroupPolicy: &trainerv1alpha1.PodGroupPolicy{
		...
		"succeeded to build PodGroup with multiple PodSets and resources": {
			info: &runtime.Info{
				Scheduler: &runtime.Scheduler{},
				RuntimePolicy: runtime.RuntimePolicy{
					PodGroupPolicy: &trainerv1alpha1.PodGroupPolicy{
		...
		"no action when PodGroup already exists and TrainJob is not suspended": {
			...
			wantInfo: &runtime.Info{
				Scheduler: &runtime.Scheduler{
					PodLabels: map[string]string{
						PodGroupLabel: "existingTrainJob",
					},
				},
				Labels: map[string]string{"key": "value"},
				RuntimePolicy: runtime.RuntimePolicy{
		...
		"uses default timeout when not specified": {
			info: &runtime.Info{
				...
				RuntimePolicy: runtime.RuntimePolicy{
					PodGroupPolicy: &trainerv1alpha1.PodGroupPolicy{
						PodGroupPolicySource: trainerv1alpha1.PodGroupPolicySource{
							KAI: &trainerv1alpha1.KAIPodGroupPolicySource{
								// No timeout specified, should use default of 60
							},
						},
					},
				},
			...
			wantObjs: []apiruntime.Object{
				utiltesting.MakeSchedulerPluginsPodGroup(metav1.NamespaceDefault, "trainJob").
					MinMember(2).
					MinResources(corev1.ResourceList{}).
					SchedulingTimeout(60). // Default timeout
					ControllerReference(trainerv1alpha1.GroupVersion.WithKind(trainerv1alpha1.TrainJobKind), "trainJob", "trainJob").
					Obj(),
			},
		},
