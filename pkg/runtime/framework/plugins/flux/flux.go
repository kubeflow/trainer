...
func (f *Flux) Build(ctx context.Context, info *runtime.Info, trainJob *trainerapi.TrainJob) ([]apiruntime.ApplyConfiguration, error) {
	...
	cm, err := generateConfigMap(js, trainJob, settings)
	if err != nil {
		return nil, err
	}
...
func (f *Flux) brokerSettingsFromTrainJob(trainJob *trainerapi.TrainJob) map[string]string {
...
func getJobSetSize(job *trainerapi.TrainJob) int32 {
	size := *job.Spec.Trainer.NumNodes
	if size < 1 {
		size = 1
	}
	return size
...
// generateConfigMap creates a ConfigMapApplyConfiguration to support server-side Apply
...
func generateFluxEntrypoint(trainJob *trainerapi.TrainJob) string {
