...
func TestClusterTrainingRuntime_ValidateObjects_DeprecatedWarning(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	t.Cleanup(cancel)

	deprecated := testingutil.MakeClusterTrainingRuntimeWrapper("test-runtime").
		RuntimeSpec(
			testingutil.MakeTrainingRuntimeSpecWrapper(
				testingutil.MakeClusterTrainingRuntimeWrapper("test-runtime").Spec,
			).Obj(),
		).Obj()
	...
	clusterRT, err := NewClusterTrainingRuntime(ctx, c, testingutil.AsIndex(clientBuilder))
	if err != nil {
		t.Fatal(err)
	}

	tj := testingutil.MakeTrainJobWrapper(metav1.NamespaceDefault, "job").
		RuntimeRef(trainer.SchemeGroupVersion.WithKind(trainer.ClusterTrainingRuntimeKind), "test-runtime").
		Obj()

	warnings, errs := clusterRT.ValidateObjects(ctx, nil, tj)
	if len(errs) != 0 {
		t.Fatalf("unexpected errors: %v", errs)
	}
	if len(warnings) == 0 {
		t.Fatalf("expected deprecation warning, got none")
	}
}
