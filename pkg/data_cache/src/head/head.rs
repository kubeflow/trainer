...
impl Distributor {
    ...
    async fn fetch_data_files(&self) -> Result<()> {
        ...
        self.ctx.register_table(self.mem_table_name.clone(), Arc::new(memtable)).map_err(|e| DataFusionError::Execution(format!("Failed to register table: {}", e)))?;
        Ok(())
    }
    ...
    async fn distribute_data_files(&self) -> Result<()> {
        ...
        let plan = DistributedWriterExec::new(Arc::new(plan), self.worker_map.clone(),
                                              self.arrow_schema.clone(), self.num_workers);
