...
pub async fn run(host: &String, port: &String, workers: Vec<String>) -> datafusion::common::Result<(), Box<dyn std::error::Error>> {
    ...
    let provider = DataFileTableProvider::new(
        dataset_config.metadata_loc,
        dataset_config.table_name,
        dataset_config.schema_name,
        schema.clone(),
        num_workers
    ...
    let mut distributor = Distributor::new(ctx, num_workers, Arc::new(provider), "memtable".to_string(), Arc::new(worker_map), schema.clone());
