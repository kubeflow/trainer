...
impl DistributedWriterExec {
    pub fn new(input: Arc<dyn ExecutionPlan>, worker_map: Arc<HashMap<String, String>>,  schema: SchemaRef, num_partitions: usize) -> Self {
...
impl ExecutionPlan for DistributedWriterExec {
    ...
    fn execute(
        ...
        let stream = futures::stream::once(send_record_batch(self.input.clone(), _context,
                                                             _partition, self.worker_map.clone())).try_flatten();
        Ok(Box::pin(RecordBatchStreamAdapter::new(self.schema.clone(), stream)))
    }
}

pub async fn send_record_batch(input: Arc<dyn ExecutionPlan>, _context: Arc<TaskContext>, _partition: usize,  worker_map: Arc<HashMap<String, String>>) -> Result<SendableRecordBatchStream> {
    ...
    let mut stream = match input.execute(_partition, _context) {
        Err(e) => {
            ...
            return Err(DataFusionError::Execution("Error with executing input plan in distributedExec".into()));
        }
        Ok(stream) => stream,
    };
        ...
        match item {
            Ok(rb) => {
                ...
                let worker_id = if let DataType::UInt64 = worker_ids.data_type() {
                    ...
                    return Err(DataFusionError::Execution("Expected UInt64 DataType".to_string()));
                };
                ...
                let mut client = ExecutorClient::try_new(addr).await?;
                let _ = client.send_batch(rb.schema(), vec![Ok(rb)]).await;
            },
            Err(error) => {
                let err = error.to_string();
                error!("Stopping execution: {err}");
                return Err(DataFusionError::Execution( "error sending batch".to_string()));
            }
        }
    }
...
impl ExecutorClient {
    pub async fn try_new(addr: &String) -> Result<Self> {
        ...
        let connection = tonic::transport::Endpoint::new(addr.clone()).map_err(|e| DataFusionError::Execution(format!("Failed to create endpoint: {}", e)))?
            .connect_timeout(Duration::from_secs(20)).timeout(Duration::from_secs(60)) //TODO: fix timeout to not allowing closing connection
            .connect()
