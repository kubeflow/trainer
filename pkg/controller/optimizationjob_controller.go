...
func (r *OptimizationJobReconciler) createTrainJobs(ctx context.Context, optJob *trainer.OptimizationJob, trainJobs []trainer.TrainJob, trialsToSpawn int32) error {
	...
	for i, paramAssignments := range suggestions {
		...
		newTrainJob, err := r.constructTrainJobWithIndex(optJob, trialIndex, paramAssignments)
		if err != nil {
...
func (r *OptimizationJobReconciler) constructTrainJob(optJob *trainer.OptimizationJob, params []trainer.ParameterAssignment) (*trainer.TrainJob, error) {
	return r.constructTrainJobWithIndex(optJob, -1, params)
}

func (r *OptimizationJobReconciler) constructTrainJobWithIndex(optJob *trainer.OptimizationJob, trialIndex int32, params []trainer.ParameterAssignment) (*trainer.TrainJob, error) {
	...
	tj := &trainer.TrainJob{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name:        optimizationjob.GenerateTrialName(optJob.Name, params),
	...
	if trialIndex >= 0 {
		tj.Name = optimizationjob.GenerateTrialNameWithIndex(optJob.Name, trialIndex, params)
	}
