...
class TrainerV1alpha1ContainerPatch(BaseModel):
    ...
    resources: Optional[IoK8sApiCoreV1ResourceRequirements] = Field(default=None, description="resources patches the container's compute resources. For the node container, resourcesPerNode on the trainer overrides patched requests and limits per key, and resourceClaimsPerNode on the trainer is placed ahead of patched claims; prefer those fields over patching the node container.")
