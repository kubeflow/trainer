...
HELM=${HELM:-helm}
TRAINER_CHART_DIR=${TRAINER_CHART_DIR:-charts/kubeflow-trainer}
...
update_rbac() {
    ...
    ${YQ} -e "select(.kind == \"ServiceAccount\" and .metadata.name == \"kubeflow-trainer-controller-manager\") | ${YQ_ARGS}" "${MANIFESTS_FILE}" >> ${DST_RBAC_DIR}/serviceaccount.yaml
    ...
    ${YQ} -e "select(.kind == \"ClusterRoleBinding\" and .metadata.name == \"kubeflow-trainer-controller-manager\") | ${YQ_ARGS}" "${MANIFESTS_FILE}" >> ${DST_RBAC_DIR}/clusterrolebinding.yaml
    ...
    ${YQ} -e "select(.kind == \"RoleBinding\" and .metadata.name == \"kubeflow-trainer-controller-manager\") | ${YQ_ARGS}" "${MANIFESTS_FILE}" >> ${DST_RBAC_DIR}/rolebinding.yaml
}
...
update_webhook() {
    ...
    ${YQ} -e "select(.kind == \"ValidatingWebhookConfiguration\" and .metadata.name == \"validator.trainer.kubeflow.org\" | ${YQ_ARGS})" "${MANIFESTS_FILE}" >> ${DST_WEBHOOK_DIR}/validatingwebhookconfiguration.yaml
}
