...
=======
>>>>>>> 16408067 (merge CPU and GPU E2E workflows into a single file)
...
# 1. Start the kustomization file with the base XGBoost image (Needed for both CPU and GPU)
...
# 2. Append the GPU-specific images if the flag is true
if [ "${GPU_CLUSTER}" = "true" ]; then
  cat <<EOF >>"${E2E_RUNTIMES_DIR}/kustomization.yaml"
...
# 3. Apply the runtimes kustomization
...
# 4. Patch CRDs to run on GPU nodes
if [ "${GPU_CLUSTER}" = "true" ]; then
