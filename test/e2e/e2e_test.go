...
var _ = ginkgo.Describe("TrainJob e2e", func() {
	...
	ginkgo.When("Creating a TrainJob with PodTemplateOverrides", func() {
		ginkgo.It("should allow the user to set the manager field and default to the caller if empty", func() {
			...
			ginkgo.By("Verify manager field preserves user input and defaults to caller", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					gotTrainJob := &trainer.TrainJob{}
					g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(trainJob), gotTrainJob)).Should(gomega.Succeed())
					g.Expect(gotTrainJob.Spec.PodTemplateOverrides).Should(gomega.HaveLen(2))
					g.Expect(*gotTrainJob.Spec.PodTemplateOverrides[0].Manager).To(gomega.Equal("kubernetes-admin"))
					g.Expect(*gotTrainJob.Spec.PodTemplateOverrides[1].Manager).To(gomega.Equal("test-manager"))
