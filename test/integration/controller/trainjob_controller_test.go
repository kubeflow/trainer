...
var _ = ginkgo.Describe("TrainJob controller", ginkgo.Ordered, func() {
	...
	ginkgo.When("Reconciling TrainJob", func() {
		...
		ginkgo.Context("Integration tests for Dynamic Resource Allocation", func() {
			...
			// newTrainJob returns a TrainJob that consumes the "gpu" claim from the given
			// ResourceClaimTemplate on every training node.
			...
			// claim from the given ResourceClaimTemplate and the node container consumes it.
			...
			ginkgo.It("Should wire resourceClaimsPerNode into the JobSet", func() {
				ginkgo.By("Creating Torch TrainingRuntime and TrainJob")
				trainingRuntime = testingutil.MakeTrainingRuntimeWrapper(ns.Name, "alpha").
				...
				ginkgo.By("Checking if the JobSet consumes the claim")
				gomega.Eventually(func(g gomega.Gomega) {
					jobSet := &jobsetv1alpha2.JobSet{}
					g.Expect(k8sClient.Get(ctx, trainJobKey, jobSet)).Should(gomega.Succeed())
			...
			ginkgo.It("Should replace a runtime resourceClaim with the same name from resourceClaimsPerNode", func() {
				ginkgo.By("Creating Torch TrainingRuntime with a gpu claim and TrainJob")
				trainingRuntime = testingutil.MakeTrainingRuntimeWrapper(ns.Name, "alpha").
