...
- `EnforceInitializer` (Build Phase): a new extension point that mutates the
  initializer `PodSet`s in `RuntimeInfo` to apply the `TrainJob` initializer
  configuration.
...
- `Build Phase`:
  ...
  - Extension Point:
    ...
    - `EnforceInitializer`: This configures initializer-specific parameters (specified in TrainJob `.spec.initializer`)
      on the dataset and model initializer PodSets.
