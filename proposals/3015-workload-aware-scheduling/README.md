...
1. Support Basic and Gang scheduling policies, topology constraints, disruption modes, and shared
   ReplicatedJob-level and Job-level resource claims.
...
reach its `minCount`. See [Sequenced Startup](#sequenced-startup).
...
//     and/or disruptionMode to configure a single PodGroup covering the whole TrainJob,
//     and leave replicatedJobs unset.
//   - the per-ReplicatedJob (level 2) model: set replicatedJobs and leave
//     schedulingPolicy, schedulingConstraints, and disruptionMode unset at the top level.
...
type Scheduling struct {
    ...
    // the TrainJob-level schedulingPolicy, schedulingConstraints, and disruptionMode fields.
    // +optional
    // +listType=atomic
...
The TrainingRuntime and ClusterTrainingRuntime validating webhooks enforce that:
...
- Setting a level 1 `schedulingPolicy`, `schedulingConstraints`, or `disruptionMode` together with
  `replicatedJobs` is rejected unless the `TrainJobCompositePodGroup` feature gate is enabled, since
  without a `CompositePodGroup` there is nothing to reconcile the two levels.
...
| Suspend                                  | Retain the Workload and PodGroups; only the Pods are deleted.                        |
