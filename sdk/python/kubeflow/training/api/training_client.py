...
class TrainingClient(object):
    ...
    def create_job(
        ...
        resolved_queue_name = self._check_queue_configuration(queue_name, labels)
        if resolved_queue_name is not None:
            queue_exists = self._check_queue_exists(resolved_queue_name, namespace)
            if queue_exists is not False:
                if job.metadata.labels is None:
                    job.metadata.labels = {}
                job.metadata.labels[constants.LOCAL_QUEUE_LABEL] = resolved_queue_name
    ...
    def _check_queue_configuration(
        ...
        """Internal method to resolve and validate queue configuration.
        ...
        Returns:
            Optional[str]: The resolved queue name to use, or None if no queue is specified.

        Raises:
            ValueError: If queue label conflict between queue_name parameter and labels.
        """
        # Get queue name from labels if present
        ...
        # Return whichever one is provided (or None if neither)
        return queue_name or queue_name_from_labels
