...
class TrainingClient(object):
    ...
    def create_job(
        ...
        queue_exists = self.validate_queue(queue_name, namespace, labels)
    ...
    def validate_queue(
        ...
        try:
            # Check if Kueue CRD is installed
            try:
                api_extensions = client.ApiextensionsV1Api(self.custom_api.api_client)
                api_extensions.read_custom_resource_definition(
                    "localqueues.kueue.x-k8s.io"
                )
                logger.debug("Kueue CRD 'localqueues.kueue.x-k8s.io' is installed.")
            except ApiException as e:
                if e.status == 404:
                    logger.warning(
                        "Kueue CRD 'localqueues.kueue.x-k8s.io' is not installed in the cluster."
                    )
                    return False
                else:
                    logger.error(f"Error while checking Kueue CRD: {e}")
                    raise
