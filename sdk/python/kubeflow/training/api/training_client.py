...
class TrainingClient(object):
    ...
    def create_job(
        ...
        queue_exists = self._validate_queue(queue_name, namespace, labels)
        if queue_name is not None and queue_exists:
            if labels is None:
                labels = {}
    ...
    def _validate_queue(
        ...
        """Internal method to validate Kueue queue configuration.
        This method is not meant to be called directly by users.
        It is used internally by create_job() and train() methods to validate
        queue configuration when either a queue name is provided or when
        labels contain a queue name.

        When neither queue_name nor labels containing a queue name are provided,
        this method returns False to indicate no queue validation is needed.

        Args:
            queue_name: Optional name of the Kueue queue provided as a parameter.
            namespace: Namespace where the queue should exist.
            labels: Optional dictionary of labels that might contain the queue name.

        Returns:
            bool: True if the queue exists, False if no queue validation is needed
                  or if the queue doesn't exist.

        Raises:
            ValueError: If queue label conflict between queue_name parameter and labels.
            RuntimeError: If there are permission issues.
        """
        # Get queue name from either parameter or labels
        queue_name_from_labels = None
        if labels is not None and constants.LOCAL_QUEUE_LABEL in labels:
            queue_name_from_labels = labels[constants.LOCAL_QUEUE_LABEL]
            ...
            try:
                ...
                if e.status == 404:
                    ...
                    raise RuntimeError(
                        ...
                        f"in namespace '{namespace}'. Please ensure you have the necessary "
                        "RBAC permissions to get/list LocalQueue resources."
                    )
                else:
                    logger.warning(
